 ...
 * - Reason: High-quality image generation model with better resolution support
...
export const generateFashionImage = async (
  ...
  return callWithRetry(async () => {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        if (errorMsg.includes('not found') || errorMsg.includes('not available') || errorMsg.includes('404')) {
          console.warn(`Model gemini-2.0-flash-exp không khả dụng (lần ${attempt}/3)`);
          if (attempt < 3) {
    ...
    if (!response) {
      throw new Error(`Model gemini-2.0-flash-exp không khả dụng sau 3 lần thử. Chi tiết lỗi: ${lastError?.message || 'Unknown error'}`);
    }
  ...
  return callWithRetry(async () => {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        const errorMsg = error.message || '';
        if (errorMsg.includes('not found') || errorMsg.includes('not available') || errorMsg.includes('404')) {
          console.warn(`Model gemini-2.0-flash-exp không khả dụng cho refine (lần ${attempt}/3)`);
          if (attempt < 3) {
            // Đợi 2 giây trước khi thử lại
            await new Promise(resolve => setTimeout(resolve, 2000));
            continue;
    ...
    // Nếu sau 3 lần vẫn không được, throw error
    if (!response) {
      throw new Error(`Model gemini-2.0-flash-exp không khả dụng sau 3 lần thử cho refine. Chi tiết lỗi: ${lastError?.message || 'Unknown error'}`);
    }
